        ...
        <table cellpadding="0" cellspacing="0" width="100%" border="0" class="listtable">
            ...
            @foreach($itemlist as $id=>$item)
                <tr id="favorite-item-{{$id}}">
                    <td><div class="pic"><a href="{{post_url($item['dataid'])}}" target="_blank"><img src="{{image_url($item['image'])}}"></a></div></td>
                    <td>
                        <h3 class="title"><a href="{{post_url($item['dataid'])}}" target="_blank">{{$item['title']}}</a></h3>
                    </td>
        ...
        $(function () {
            $("a[rel=a-delete]").confirm({
                ...
                onConfirm:function(view,o){
                    ...
                    var spinner = null;
                    $.ajax({
                        url:'{{url('/user/collection/delete')}}',
                        data:{id:id},
                        dataType:"json",
                        beforeSend:function () {
