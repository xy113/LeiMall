...
@section('content')
    <div class="console-title">
        <div class="float-right">
            <label><a href="/admin/job" class="button">职位列表</a></label>
        </div>
        <h2>职位管理->职位列表</h2>
    </div>
        ...
        <form method="post" id="companyForm">
            ...
            <table cellpadding="0" cellspacing="0" width="100%" class="formtable">
                ...
                <tr>
                    <td>招聘单位</td>
                    <td colspan="2">
                        <select title="" class="select" name="job[company_id]">
                            @foreach($companylist as $company)
                            <option value="{{$company['company_id']}}"@if($company['company_id']==$job['company_id']) selected="selected"@endif>{{$company['company_name']}}</option>
                            @endforeach
                        </select>
                    </td>
                </tr>
