    ...
    <div class="content-div">
        <form method="post" id="settingForm" action="{{url('admin/settings/save')}}">
            ...
            <table class="formtable" border="0" cellpadding="0" cellspacing="0" width="100%">
                <tbody id="water">
                <tr>
                    <td class="cell-label" width="90">开启图片水印:</td>
                    <td width="320">
                        <label><input name="settings[water_mark]" value="1" type="radio" @if ($settings['water_mark']) checked="checked"@endif> 是</label>
                        <label><input name="settings[water_mark]" value="0" type="radio" @if (!$settings['water_mark']) checked="checked"@endif> 否</label>
                    </td>
                    <td>是否启用图片水印</td>
                </tr>
                <tr>
                    ...
                    <td>
                        <label><input name="settings[water_type]" type="radio" value="1"@if($settings['water_type']) checked="checked"@endif> 图片水印</label>
                ...
                <tr>
                    ...
                    <td>
                        <select title="" class="form-control w300" name="settings[water_pos]">
                ...
                <tr>
                    ...
                    <td><input title="" name="settings[water_alpha]" value="{{$settings['water_alpha'] or ''}}" class="form-control w300" type="text"></td>
                    <td>水印透明度,100为不透明</td>
                </tr>
                <tr>
                    <td class="cell-label">水印文字:</td>
                    <td><textarea title="" name="settings[water_text]" class="form-control w300">{{$settings['water_text'] or ''}}</textarea></td>
                    <td>水印文字，仅当设定为文字水印时有效</td>
                </tr>
                <tr>
                    <td class="cell-label">水印颜色:</td>
                    <td><input title="" name="settings[water_color]" value="{{$settings['water_color'] or ''}}" class="form-control w300" type="text"></td>
                    <td>水印文字颜色，仅当设定为文字水印时有效</td>
                </tr>
                <tr>
                    <td class="cell-label">字体大小:</td>
                    <td><input title="" name="settings[water_size]" value="{{$settings['water_size'] or ''}}" class="form-control w300" type="text"></td>
                    <td>水印文字字体大小，仅当设定为文字水印时有效</td>
                </tr>
                <tr>
                    <td class="cell-label">位置偏移量:</td>
                    <td><input title="" name="settings[water_offset]" value="{{$settings['water_offset'] or ''}}" class="form-control w300" type="text"></td>
                    <td>水印位置偏移量，单位像素</td>
                </tr>
                <tr>
                    <td class="cell-label">旋转角度:</td>
                    <td><input title="" name="settings[water_angle]" value="{{$settings['water_angle'] or ''}}" class="form-control w300" type="text"></td>
                    <td>水印文字旋转角度</td>
                </tr>
                </tbody>
                <tfoot>
