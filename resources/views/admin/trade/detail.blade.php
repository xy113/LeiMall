...
@section('content')
    ...
    <div class="order-container">
        ...
        <div class="address-row">
            <label>配送方式:</label>
            <div class="content">{$_lang[shipping_type_option][$order[shipping_type]]}</div>
        </div>
        <div class="address-row">
            <label>收货地址:</label>
            <div class="content">{{$order['consignee']}}&nbsp;&nbsp;{{$order['phone']}}&nbsp;&nbsp;{{$order['address']}}</div>
        </div>
        ...
        <table cellspacing="0" cellpadding="0" width="100%" class="detail-table">
            <tr>
                <td>
                    <label>店铺名称:</label>
                    <div class="content"><a href="{{shop_url($order['shop_id'])}}" target="_blank">{{$shop['shop_name']}}</a></div>
                </td>
                <td>
                    <label>卖家账号:</label>
                ...
                <td>
                    <label>联系电话:</label>
                    <div class="content">{{$shop['phone']}}</div>
                </td>
            </tr>
            <tr>
                <td>
                    <label>所在地:</label>
                    <div class="content">{{$shop['province']}} {{$shop['city']}} {{$shop['county']}}</div>
                </td>
                <td></td>
                <td></td>
            </tr>
        ...
        <table cellspacing="0" cellpadding="0" width="100%" class="detail-table">
            <tr>
                ...
                <td>
                    <label>创建时间:</label>
                    <div class="content">{{@date('Y-m-d H:i:s', $order['created_at'])}}</div>
                </td>
            </tr>
            <tr>
                <td>
                    <label>付款时间:</label>
                    <div class="content">{{@date('Y-m-d H:i:s', $order['pay_at'])}}</div>
                </td>
                <td>
                    <label>发货时间:</label>
                    <div class="content">{{@date('Y-m-d H:i:s', $order['shipping_at'])}}</div>
                </td>
                <td></td>
            </tr>
        </table>
